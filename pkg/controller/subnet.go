	...
	if subnet.Spec.Vlan != "" && subnet.Spec.Vpc == util.DefaultVpc {
		...
		if subnet.Spec.GatewayType == kubeovnv1.GWDistributedType {
			...
			if c.config.EnableEcmp {
				...
				if err = c.addPolicyRouteForCentralizedSubnet(subnet, nodeIPs); err != nil {
				...
				nodeIPs = append(nodeIPs, strings.Split(nextHop, ",")...)
				if err = c.addPolicyRouteForCentralizedSubnet(subnet, nodeIPs); err != nil {
...
func (c *Controller) addPolicyRouteForCentralizedSubnet(subnet *kubeovnv1.Subnet, nodeIPs []string) error {
	for _, cidr := range strings.Split(subnet.Spec.CIDRBlock, ",") {
		var nextHops []string
		for _, nodeIp := range nodeIPs {
			if util.CheckProtocol(nodeIp) != util.CheckProtocol(cidr) {
				continue
			}
		...
		if err := c.ovnClient.AddPolicyRoute(c.config.ClusterRouter, util.CentralSubnetPriority, match, "reroute", nextHopIp); err != nil {
			klog.Errorf("failed to add policy route for centralized subnet %s: %v", subnet.Name, err)
			return err
		}
	}
	return nil
...
func (c *Controller) deletePolicyRouteForDistributedSubnet(subnet *kubeovnv1.Subnet, node *v1.Node) error {
	nodeIP, err := getNodeTunlIP(node)
	if err != nil {
		klog.Errorf("failed to get node %s tunnel ip, %v", node.Name, err)
		return err
...
func (c *Controller) deletePolicyRouteByGatewayType(subnet *kubeovnv1.Subnet, gatewayType string) error {
	...
	if gatewayType == kubeovnv1.GWDistributedType {
		...
		for _, node := range nodes {
			...
			if err = c.deletePolicyRouteForDistributedSubnet(subnet, node); err != nil {
